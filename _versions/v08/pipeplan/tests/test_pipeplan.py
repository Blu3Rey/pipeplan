...
# --------------------------------------------------------------------------- #
# load phase: dialect-native upsert + COPY fast-path
# --------------------------------------------------------------------------- #


def test_native_upsert_updates_and_preserves_target_only_columns(tmp_path):
    a = _db(tmp_path)
    contract = DataframeContract.model_validate({
        "primary_key": ["id"],
    ...
    a.write(pd.DataFrame({"id": [1, 2], "name": ["a", "b"], "audit": ["k1", "k2"]}),
            "dim", mode=LoadMode.REPLACE, contract=contract)
    # incoming lacks `audit`; native ON CONFLICT updates name but preserves audit
    r = a.write(pd.DataFrame({"id": [2, 3], "name": ["B2", "c"]}), "dim", mode=LoadMode.UPSERT, key=["id"])
    out = a.read("dim").sort_values("id").set_index("id")
    assert out.loc[2, "name"] == "B2"
    assert out.loc[2, "audit"] == "k2"          # target-only column preserved
    assert r.updated == 1 and r.inserted == 1   # metrics split correctly
    a.dispose()


def test_upsert_fallback_without_constraint(tmp_path):
    a = _db(tmp_path)
    # plain to_sql via replace-created table has no PK -> portable fallback path
    a.write(pd.DataFrame({"k": [1, 2], "v": [10, 20], "audit": ["keep1", "keep2"]}),
            "t", mode=LoadMode.REPLACE)
    from pipeplan.adapters.sql import SqlLoadTarget
    with a.engine.begin() as conn:
        assert not SqlLoadTarget(conn, resource="t").supports_native_upsert("t", ["k"])
    # incoming omits `audit`: a real upsert updates v and preserves audit (a
    # delete+insert would null it) and never removes the unmatched row k=1.
    r = a.write(pd.DataFrame({"k": [2, 3], "v": [99, 30]}), "t", mode=LoadMode.UPSERT, key=["k"])
    got = a.read("t").sort_values("k").set_index("k")
    assert got.loc[2, "v"] == 99 and got.loc[1, "v"] == 10 and got.loc[3, "v"] == 30
    assert got.loc[2, "audit"] == "keep2"     # target-only column preserved on update
    assert pd.isna(got.loc[3, "audit"])       # newly inserted row
    assert r.updated == 1 and r.inserted == 1
    a.dispose()


def test_portable_upsert_sql_never_deletes(tmp_path):
    from pipeplan.adapters.sql import SqlLoadTarget
    a = _db(tmp_path)
    with a.engine.begin() as conn:
        t = SqlLoadTarget(conn, resource="x")
        upd = t.update_matched_sql("dim", ["id", "name", "note"], ["id"])
        ins = t.insert_missing_sql("dim", ["id", "name", "note"], ["id"])
        assert upd is not None and upd.startswith("UPDATE") and "DELETE" not in upd
        assert "WHERE EXISTS" in upd
        assert ins.startswith("INSERT INTO") and "NOT EXISTS" in ins and "DELETE" not in ins
        # all-key frames have nothing to update
        assert t.update_matched_sql("dim", ["id"], ["id"]) is None
    a.dispose()


def test_upsert_dedupes_incoming_keys(tmp_path):
    a = _db(tmp_path)
    contract = DataframeContract.model_validate({
        "primary_key": ["k"], "columns": {"k": {"dtype": "integer"}, "v": {"dtype": "integer"}}})
    a.write(pd.DataFrame({"k": [1], "v": [1]}), "t", mode=LoadMode.REPLACE, contract=contract)
    # two rows with the same key in one batch -> last wins, no PK violation
    a.write(pd.DataFrame({"k": [1, 1], "v": [5, 9]}), "t", mode=LoadMode.UPSERT, key=["k"])
    got = a.read("t")
    assert len(got) == 1 and got["v"].item() == 9
    a.dispose()


def test_native_upsert_sql_per_dialect():
    from pipeplan.adapters.sql import resolve_dialect
    q = lambda s: f'"{s}"'  # noqa: E731 - test quoter
    pg = resolve_dialect("postgresql")(q).native_upsert("dim", ["id", "name"], ["id"]).format(staging='"s"')
    assert "ON CONFLICT" in pg and "excluded" in pg and "WHERE true" in pg
    my = resolve_dialect("mysql")(q).native_upsert("dim", ["id", "name"], ["id"]).format(staging="`s`")
    assert "ON DUPLICATE KEY UPDATE" in my
    ms = resolve_dialect("mssql")(q).native_upsert("dim", ["id", "name"], ["id"]).format(staging="[s]")
    assert ms.startswith("MERGE INTO") and "WHEN NOT MATCHED" in ms


def test_ansi_and_access_dialect_resolution():
    from pipeplan.adapters.sql import AccessDialect, SqlDialect, resolve_dialect
    assert resolve_dialect("postgresql").__name__ == "PostgresDialect"
    assert resolve_dialect("access") is AccessDialect
    # an unregistered/niche backend transparently falls back to portable ANSI
    assert resolve_dialect("firebird") is SqlDialect


def test_access_dialect_uses_join_forms_not_subqueries():
    from pipeplan.adapters.sql import resolve_dialect
    q = lambda s: f"[{s}]"  # noqa: E731 - Access-style quoter
    acc = resolve_dialect("access")(q)
    # no single-statement upsert on Access -> simulation is used
    assert acc.has_native_upsert is False
    assert acc.native_upsert("dim", ["id", "name"], ["id"]) is None
    # UPDATE and DELETE must be JOIN action-queries (Access rejects subquery forms)
    upd = acc.update_matched("dim", ["id", "name"], ["id"]).format(staging="[s]")
    assert "INNER JOIN" in upd and "EXISTS" not in upd
    dele = acc.delete_matching("dim", ["id"]).format(staging="[s]")
    assert dele.startswith("DELETE") and "INNER JOIN" in dele and "EXISTS" not in dele
    ins = acc.insert_missing("dim", ["id", "name"], ["id"]).format(staging="[s]")
    assert "LEFT JOIN" in ins and "IS NULL" in ins
    # booleans are -1 / 0
    assert acc.true_literal == "-1"
    cur = acc.select_current("dim", ["id"], "is_current")
    assert "= -1" in cur


def test_ansi_dialect_is_portable_default():
    from pipeplan.adapters.sql import SqlDialect
    q = lambda s: f'"{s}"'  # noqa: E731
    ansi = SqlDialect(q)
    assert ansi.native_upsert("t", ["k", "v"], ["k"]) is None      # no native construct
    upd = ansi.update_matched("t", ["k", "v"], ["k"]).format(staging='"s"')
    assert "UPDATE" in upd and "EXISTS" in upd                     # correlated-subquery form
    assert ansi.true_literal == "1"


def test_copy_fast_path_skipped_on_sqlite(tmp_path):
    # COPY is postgres-only; sqlite append must go through the portable path.
    from pipeplan.adapters.sql import SqlLoadTarget
    a = _db(tmp_path)
    with a.engine.begin() as conn:
        target = SqlLoadTarget(conn, resource="x")
        assert target.dialect == "sqlite"
        assert target._try_copy(pd.DataFrame({"a": [1]}), "nope") is False
    a.dispose()


def test_upsert_rejects_null_keys(tmp_path):
    a = _db(tmp_path)
    a.write(pd.DataFrame({"k": [1], "v": [1]}), "t", mode=LoadMode.REPLACE)
    with pytest.raises(Exception):
        a.write(pd.DataFrame({"k": [None], "v": [9]}), "t", mode=LoadMode.UPSERT, key=["k"])
    a.dispose()


def test_upsert_rejects_unknown_columns(tmp_path):
    a = _db(tmp_path)
    a.write(pd.DataFrame({"k": [1], "v": [1]}), "t", mode=LoadMode.REPLACE)
    with pytest.raises(Exception):
        a.write(pd.DataFrame({"k": [1], "v": [2], "surprise": ["x"]}), "t",
                mode=LoadMode.UPSERT, key=["k"])
    a.dispose()


# --------------------------------------------------------------------------- #
# adapter registry (the extract-and-load extension point)
# --------------------------------------------------------------------------- #


def test_adapter_registry_lists_builtins():
    from pipeplan.core.registry import ADAPTERS
    import pipeplan.adapters  # ensure registration
    names = set(ADAPTERS.names())
    assert {"file", "db", "document"} <= names


def test_unknown_adapter_kind_errors_clearly():
    res = ResourceConfig.model_validate({"name": "r", "adapter": "definitely_not_real",
                                         "params": {"path": "/tmp/x"}})
...
def test_custom_adapter_can_be_registered():
    ...
    from pipeplan.adapters.base import Adapter

    @register_adapter("memory_test")
    class _Mem(Adapter):
        def read(self, collection, *, since=None):
            return pd.DataFrame()
        def write_batch(self, requests):
            return []

    res = ResourceConfig.model_validate({"name": "m", "adapter": "memory_test", "params": {}})
    assert isinstance(create_adapter(res), _Mem)


def test_resourceconfig_accepts_enum_and_str_kinds():
    from pipeplan.config.models import AdapterKind
    r1 = ResourceConfig.model_validate({"adapter": "file", "params": {"format": "csv", "path": "/tmp/x"}})
    r2 = ResourceConfig.model_validate({"adapter": AdapterKind.DB, "params": {"engine": "sqlite", "path": "/tmp/x"}})
    assert r1.adapter == "file" and r2.adapter == "db"


# --------------------------------------------------------------------------- #
# document (NoSQL / JSON) adapter
# --------------------------------------------------------------------------- #


def _doc(tmp_path, allow=("read", "write")):
    res = ResourceConfig.model_validate({
        "name": "docs", "adapter": "document", "allow": list(allow),
        "params": {"path": str(tmp_path / "store")},
    })
    return create_adapter(res)


def test_document_nested_roundtrip(tmp_path):
    a = _doc(tmp_path)
    # write flat dotted columns -> stored as nested documents -> read back nested
    a.write(pd.DataFrame({"id": [1], "addr.city": ["NYC"], "addr.zip": ["10001"]}),
            "people", mode=LoadMode.REPLACE)
    raw = json.loads((tmp_path / "store" / "people.json").read_text())
    assert raw == [{"id": 1, "addr": {"city": "NYC", "zip": "10001"}}]
    back = a.read("people")
    assert set(back.columns) >= {"id", "addr.city", "addr.zip"}
    assert back.loc[0, "addr.city"] == "NYC"


def test_document_upsert_by_key(tmp_path):
    a = _doc(tmp_path)
    a.write(pd.DataFrame({"id": [1, 2], "v": ["a", "b"]}), "c", mode=LoadMode.REPLACE)
    r = a.write(pd.DataFrame({"id": [2, 3], "v": ["B", "c"]}), "c", mode=LoadMode.UPSERT, key=["id"])
    got = a.read("c").sort_values("id").set_index("id")["v"].to_dict()
    assert got == {1: "a", 2: "B", 3: "c"}
    assert r.updated == 1 and r.inserted == 1


def test_document_delete_by_key(tmp_path):
    a = _doc(tmp_path)
    a.write(pd.DataFrame({"id": [1, 2, 3]}), "c", mode=LoadMode.REPLACE)
    r = a.write(pd.DataFrame({"id": [2]}), "c", mode=LoadMode.DELETE, key=["id"])
    assert sorted(a.read("c")["id"].tolist()) == [1, 3]
    assert r.deleted == 1


def test_document_composite_key_upsert(tmp_path):
    a = _doc(tmp_path)
    a.write(pd.DataFrame({"a": [1, 1], "b": ["x", "y"], "v": [1, 2]}), "c", mode=LoadMode.REPLACE)
    a.write(pd.DataFrame({"a": [1], "b": ["y"], "v": [99]}), "c", mode=LoadMode.UPSERT, key=["a", "b"])
    got = a.read("c").sort_values(["a", "b"])
    assert got.loc[got["b"] == "y", "v"].item() == 99
    assert got.loc[got["b"] == "x", "v"].item() == 1


def test_document_append_and_since(tmp_path):
    a = _doc(tmp_path)
    a.write(pd.DataFrame({"id": [1], "ts": ["2026-01-01"]}), "c", mode=LoadMode.REPLACE)
    a.write(pd.DataFrame({"id": [2], "ts": ["2026-01-05"]}), "c", mode=LoadMode.APPEND)
    assert len(a.read("c")) == 2
    assert a.read("c", since=("ts", "2026-01-02"))["id"].tolist() == [2]


def test_document_scd2_rejected(tmp_path):
    a = _doc(tmp_path)
    with pytest.raises(Exception):
        a.write(pd.DataFrame({"id": [1], "v": [1]}), "c", mode=LoadMode.SCD2, key=["id"],
                scd={"track": ["v"]})


def test_document_end_to_end_pipeline(tmp_path):
    # a document source -> transform -> document sink, no SQL involved
    store = tmp_path / "store"
    store.mkdir()
    (store / "events.json").write_text(json.dumps([
        {"id": 1, "user": {"name": "amy"}, "amount": 10},
        {"id": 2, "user": {"name": "bo"}, "amount": 5},
    ]))
    cfg = load_config({
        "metadata": {"id": "docpipe"},
        "resources": {
            "src": {"adapter": "document", "params": {"path": str(store)}, "allow": ["read"]},
            "out": {"adapter": "document", "params": {"path": str(tmp_path / "out")}, "allow": ["read", "write"]},
        },
        "tasks": {
            "ext": {"stage": "extract", "resource": "src",
                    "steps": [{"collection": "events", "output": "raw"}]},
            "tr": {"stage": "transform", "input": "raw", "output": "big",
                   "steps": [{"action": "filter", "with": {"amount": {"op": ">=", "value": 10}}}]},
            "ld": {"stage": "load", "resource": "out",
                   "steps": [{"input": "big", "collection": "big_events", "mode": "upsert", "key": ["id"]}]},
        },
    })
    run_pipeline(cfg)
    out = json.loads((tmp_path / "out" / "big_events.json").read_text())
    assert out == [{"id": 1, "user": {"name": "amy"}, "amount": 10}]  # nested doc rebuilt


# --------------------------------------------------------------------------- #
# REST adapter (mocked transport -- no network)
# --------------------------------------------------------------------------- #
...
class MockTransport:
    """Deterministic transport: a handler maps (method, url, params, json) -> Response."""

    def __init__(self, handler):
        from pipeplan.adapters.rest.transport import Response  # local import
    ...
    def request(self, method, url, *, headers=None, params=None, json=None, timeout=None):
        ...
        result = self.handler(method, url, params, json)
        return result


def _rest(params, handler, allow=("read", "write")):
    from pipeplan.adapters.rest import RestAdapter
    from pipeplan.adapters.rest.transport import Response  # noqa: F401
    base = {"base_url": "https://api.test/v1"}
    base.update(params)
    res = ResourceConfig.model_validate({"name": "api", "adapter": "rest",
                                         "allow": list(allow), "params": base})
    mock = MockTransport(handler)
    return RestAdapter(res, transport=mock), mock


def _resp(status, body=None, headers=None):
    from pipeplan.adapters.rest.transport import Response
    return Response(status, body, headers or {})


def test_rest_read_single_and_nested():
    def handler(method, url, params, json):
        return _resp(200, {"data": [{"id": 1, "user": {"name": "amy"}}, {"id": 2, "user": {"name": "bo"}}]})
    a, _ = _rest(
        {"endpoints": {"contacts": {"read": {"method": "GET", "path": "/contacts", "record_path": "data"}}}},
        handler)
    df = a.read("contacts")
    assert df["id"].tolist() == [1, 2]
    assert df["user.name"].tolist() == ["amy", "bo"]     # nested flattened


def test_rest_read_page_pagination():
    pages = {1: [{"id": 1}], 2: [{"id": 2}], 3: []}
    def handler(method, url, params, json):
        return _resp(200, {"data": pages[params["page"]]})
    a, mock = _rest(
        {"pagination": {"style": "page", "param": "page"},
         "endpoints": {"c": {"read": {"method": "GET", "path": "/c", "record_path": "data"}}}},
        handler)
    df = a.read("c")
    assert df["id"].tolist() == [1, 2]                   # concatenated, stops on empty page


def test_rest_read_cursor_pagination_and_since():
    def handler(method, url, params, json):
        if "cursor" not in params:
            return _resp(200, {"data": [{"id": 1}], "meta": {"next": "abc"}})
        return _resp(200, {"data": [{"id": 2}], "meta": {"next": None}})
    a, mock = _rest(
        {"pagination": {"style": "cursor", "param": "cursor", "next": "meta.next"},
         "endpoints": {"c": {"read": {"method": "GET", "path": "/c", "record_path": "data",
                                      "since_param": "updated_since"}}}},
        handler)
    df = a.read("c", since=("ts", "2026-01-01"))
    assert df["id"].tolist() == [1, 2]
    assert mock.calls[0]["params"]["updated_since"] == "2026-01-01"   # incremental param sent


def test_rest_append_posts_with_idempotency():
    def handler(method, url, params, json):
        return _resp(201, {"ok": True})
    a, mock = _rest(
        {"concurrency": 1,
         "endpoints": {"c": {"key": "id", "create": {"method": "POST", "path": "/c"}}}},
        handler)
    r = a.write(pd.DataFrame({"id": [1], "name": ["a"]}), "c", mode=LoadMode.APPEND)
    assert r.inserted == 1
    assert mock.calls[0]["method"] == "POST"
    assert mock.calls[0]["json"] == {"id": 1, "name": "a"}
    assert "Idempotency-Key" in mock.calls[0]["headers"]


def test_rest_upsert_patches_then_falls_back_to_create():
    def handler(method, url, params, json):
        if method == "PATCH" and url.endswith("/2"):
            return _resp(200, {})           # existing -> updated
        if method == "PATCH" and url.endswith("/3"):
            return _resp(404, {})           # missing -> triggers create fallback
        if method == "POST":
            return _resp(201, {})
        return _resp(500, {})
    a, mock = _rest(
        {"concurrency": 1,
         "endpoints": {"c": {"key": "id",
                             "update": {"method": "PATCH", "path": "/c/{id}"},
                             "create": {"method": "POST", "path": "/c"}}}},
        handler)
    r = a.write(pd.DataFrame({"id": [2, 3], "v": ["x", "y"]}), "c", mode=LoadMode.UPSERT, key=["id"])
    assert r.updated == 1 and r.inserted == 1
    methods = [c["method"] for c in mock.calls]
    assert "PATCH" in methods and "POST" in methods


def test_rest_delete_is_idempotent_on_404():
    def handler(method, url, params, json):
        return _resp(404, {})               # already gone
    a, _ = _rest(
        {"concurrency": 1,
         "endpoints": {"c": {"key": "id", "delete": {"method": "DELETE", "path": "/c/{id}"}}}},
        handler)
    r = a.write(pd.DataFrame({"id": [9]}), "c", mode=LoadMode.DELETE, key=["id"])
    assert r.deleted == 1


def test_rest_replace_refused_without_replace_op():
    a, _ = _rest(
        {"endpoints": {"c": {"key": "id", "create": {"method": "POST", "path": "/c"}}}},
        lambda *a: _resp(200, {}))
    with pytest.raises(Exception) as exc:
        a.write(pd.DataFrame({"id": [1]}), "c", mode=LoadMode.REPLACE, key=["id"])
    assert "refused" in str(exc.value).lower()


def test_rest_scd2_rejected_and_undeclared_endpoint():
    a, _ = _rest({"endpoints": {"c": {"read": {"method": "GET", "path": "/c"}}}}, lambda *a: _resp(200, {}))
    with pytest.raises(Exception):
        a.write(pd.DataFrame({"id": [1]}), "c", mode=LoadMode.SCD2, key=["id"], scd={"track": ["v"]})
    with pytest.raises(Exception) as exc:
        a.read("nope")
    assert "not declared" in str(exc.value)


def test_rest_permission_blocks_write():
    a, _ = _rest(
        {"endpoints": {"c": {"key": "id", "create": {"method": "POST", "path": "/c"}}}},
        lambda *a: _resp(201, {}), allow=("read",))
    with pytest.raises(PermissionDeniedError):
        a.write(pd.DataFrame({"id": [1]}), "c", mode=LoadMode.APPEND)


def test_rest_dry_run_sends_nothing():
    a, mock = _rest(
        {"dry_run": True,
         "endpoints": {"c": {"key": "id", "create": {"method": "POST", "path": "/c"}}}},
        lambda *a: _resp(201, {}))
    r = a.write(pd.DataFrame({"id": [1, 2]}), "c", mode=LoadMode.APPEND)
    assert r.inserted == 2 and mock.calls == []          # nothing actually sent


def test_rest_auth_bearer_and_apikey_param():
    a, mock = _rest(
        {"auth": {"type": "bearer", "token": "SEKRET"},
         "endpoints": {"c": {"read": {"method": "GET", "path": "/c", "record_path": "data"}}}},
        lambda *a: _resp(200, {"data": []}))
    a.read("c")
    assert mock.calls[0]["headers"]["Authorization"] == "Bearer SEKRET"

    a2, mock2 = _rest(
        {"auth": {"type": "api_key", "key": "K123", "param": "api_key"},
         "endpoints": {"c": {"read": {"method": "GET", "path": "/c", "record_path": "data"}}}},
        lambda *a: _resp(200, {"data": []}))
    a2.read("c")
    assert mock2.calls[0]["params"]["api_key"] == "K123"  # key injected as query param


def test_rest_retry_on_429(monkeypatch):
    import pipeplan.adapters.rest.transport as tr
    monkeypatch.setattr(tr.time, "sleep", lambda *_: None)   # no real waiting
    state = {"n": 0}
    def handler(method, url, params, json):
        state["n"] += 1
        if state["n"] == 1:
    ...
    a, _ = _rest(
        {"concurrency": 1,
         "endpoints": {"c": {"key": "id", "create": {"method": "POST", "path": "/c"}}}},
        handler)
    r = a.write(pd.DataFrame({"id": [1]}), "c", mode=LoadMode.APPEND)
    assert r.inserted == 1 and state["n"] == 2               # retried once, then succeeded


def test_rest_config_validation_errors():
    from pipeplan.adapters.rest import RestAdapter
    # bad HTTP method
    bad = ResourceConfig.model_validate({"name": "x", "adapter": "rest", "params": {
        "base_url": "https://a.test", "endpoints": {"c": {"read": {"method": "FETCH", "path": "/c"}}}}})
    with pytest.raises(Exception):
        RestAdapter(bad, transport=MockTransport(lambda *a: _resp(200, {})))
    # update op without a key
    bad2 = ResourceConfig.model_validate({"name": "x", "adapter": "rest", "params": {
        "base_url": "https://a.test", "endpoints": {"c": {"update": {"method": "PATCH", "path": "/c/{id}"}}}}})
    with pytest.raises(Exception):
        RestAdapter(bad2, transport=MockTransport(lambda *a: _resp(200, {})))


def test_rest_read_transform_write_flow():
    # exercise read -> (transform) -> write against the mock, the way a pipeline would
    src_docs = [{"id": 1, "amount": 10}, {"id": 2, "amount": 3}]
    posted = []
    def handler(method, url, params, json):
        if method == "GET":
            return _resp(200, {"data": src_docs})
        posted.append(json)
        return _resp(201, {})
    a, _ = _rest(
        {"concurrency": 1,
         "endpoints": {"orders": {"key": "id",
                                  "read": {"method": "GET", "path": "/orders", "record_path": "data"},
                                  "create": {"method": "POST", "path": "/orders"}}}},
        handler)
    df = a.read("orders")
    big = build_transform("filter", {"column": "amount", "op": ">=", "value": 10}).apply(df, _ctx())
    a.write(big, "orders", mode=LoadMode.APPEND)
    assert posted == [{"id": 1, "amount": 10}]
