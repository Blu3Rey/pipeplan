...
class ResourceConfig(StrictModel):
    """A named external system (a file, a database, a document store, ...)."""

    name: str | None = None
    #: adapter kind, resolved against the ``pipeplan.adapters`` registry at
    #: provision time. Open by design so non-built-in resources (document/NoSQL,
    #: APIs, object stores) are first-class without a schema change.
    adapter: str
    params: dict[str, Any]
    ...
    @classmethod
    def _kind_to_str(cls, value: Any) -> Any:
