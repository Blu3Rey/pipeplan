...
Adapters are the extract-and-load extension point, resolved from a registry
(entry-point group `pipeplan.adapters`) by the resource's `adapter` kind — an
open string, so new resource families are first-class without a schema change.
Bundled families:

- **`file`** — pandas file I/O (excel/csv/tsv/json/parquet); `replace`/`append`,
  atomic (temp-file + rename), column-aligned appends.
- **`db`** — relational, via SQLAlchemy, for everything from SQLite/Access to
  Postgres/MySQL/SQL Server; the full set of load modes plus dialect compilers.
  Lives in its own `adapters/sql/` subpackage.
- **`document`** — JSON / NoSQL document store (the shape of MongoDB, DynamoDB,
  Firestore, CouchDB). Nested documents flatten to dotted columns on read and
  are rebuilt on write; `replace`/`append`/`upsert`/`delete` are implemented
  natively per-document by key (no SQL). `scd2` is rejected as a relational
  pattern.
- **`rest`** / **`http`** — a REST API as a resource. GET endpoints extract into
  DataFrames (with page/offset/cursor/link pagination and an incremental `since`
  query param); load modes map to verbs — `append`→POST, `upsert`→PATCH/PUT by
  key with POST fallback, `delete`→DELETE by key. Safety is enforced by a
  declarative **endpoint map**: only declared operations are callable, each with a
  pinned method and a path template rooted at `base_url`, so an unmapped URL or an
  accidental destructive verb is impossible. `replace` is refused unless an
  endpoint opts in; `scd2` is rejected. Adds bearer/api-key/basic auth,
  rate-limiting, retry with `Retry-After`, idempotency keys, `dry_run`, and
  concurrent per-record writes. Needs `pip install 'pipeplan[rest]'`.

The contract is storage-agnostic — `read(collection) -> DataFrame` and
`write_batch(requests)` over the shared `LoadMode` vocabulary — so the load
*modes* mean the same thing across families while each implements them in its own
terms. A resource's `allow: [read, write]` list is enforced in the adapter. A new
backend (a REST API, an object store, another database) is one registered
`Adapter` subclass, addable out-of-tree.

A REST resource declares its connection and a safety-first endpoint map:
...
resources:
  crm_api:
    ...
    allow: [read, write]
    params:
      base_url: "https://api.crm.example.com/v2"    # host is pinned here
      auth: { type: bearer, token: "${secret:crm/token}" }
      rate_limit: { requests: 10, per: 1s }
      retry: { attempts: 4, backoff: 2, on_status: [429, 500, 502, 503] }
      pagination: { style: cursor, param: cursor, next: meta.next_cursor }
      endpoints:
        contacts:
          key: id
          read:   { method: GET,    path: /contacts, record_path: data, since_param: updated_since }
          create: { method: POST,   path: /contacts }
          update: { method: PATCH,  path: /contacts/{id} }
          delete: { method: DELETE, path: /contacts/{id} }
```

A step's `collection: contacts` resolves to a declared endpoint; the load `mode`
selects the pinned verb. Undeclared endpoints and unmapped modes fail fast.
...
- **Load metrics.** Each write returns rows inserted/updated/deleted, logged per
  step.
- **Dialect-native fast-paths.** `upsert` is a true update-or-insert that never
  deletes. When the key is backed by a unique/primary-key constraint it uses the
  native construct — `INSERT ... ON CONFLICT` (SQLite/Postgres), `ON DUPLICATE
  KEY` (MySQL), `MERGE` (SQL Server). Otherwise the same semantics run portably:
  `UPDATE` the matched rows (only the incoming non-key columns, so target-only
  columns survive) then `INSERT ... WHERE NOT EXISTS` the rest. Either path
  preserves target-only columns and fires update/insert triggers correctly. On
  Postgres, bulk appends use `COPY` when psycopg2 is available. Upsert inputs are
  validated up front: null keys and columns absent from the target are rejected
  with a clear error rather than an opaque SQL failure.
...
The set-based SQL each load mode emits is produced by a **dialect compiler**, not
inlined per operation. `SqlLoadTarget` owns the mechanics (connection, staging,
introspection); a `SqlDialect` owns the *spelling* of each statement. Compilers
are selected per connection by SQLAlchemy dialect name from a registry
(entry-point group `pipeplan.sql_dialects`), falling back to a portable ANSI
compiler for anything unregistered.

Mainstream backends (SQLite, PostgreSQL, MySQL, SQL Server) each contribute only
their native-upsert spelling and otherwise share the ANSI correlated-subquery
forms. A niche backend is a self-contained subclass that overrides only what
diverges and touches no shared code — the bundled `AccessDialect` (Microsoft
Access/Jet) overrides `UPDATE`/`DELETE`/close-version to `INNER JOIN`
action-queries (Access rejects the subquery forms), uses `-1`/`0` booleans, and
declares no native upsert (so it uses the portable update-then-insert
simulation). Adding support for another niche database — Firebird, Teradata,
DuckDB — is one registered subclass, out-of-tree if you like.
...
firebird = "my_pkg.module:FirebirdDialect"

[project.entry-points."pipeplan.adapters"]
mongodb = "my_pkg.module:MongoAdapter"
```
