...
LOAD_STRATEGY_GROUP = "pipeplan.load_strategies"
SQL_DIALECT_GROUP = "pipeplan.sql_dialects"
ADAPTER_GROUP = "pipeplan.adapters"
...
LOAD_STRATEGIES: "Registry[type]" = Registry(LOAD_STRATEGY_GROUP, "load strategy")
SQL_DIALECTS: "Registry[type]" = Registry(SQL_DIALECT_GROUP, "sql dialect")
ADAPTERS: "Registry[type]" = Registry(ADAPTER_GROUP, "adapter")


def register_adapter(kind: str):
    """Register a resource adapter under a kind name (e.g. 'file', 'db', 'document').

    Adapters are the extract-and-load extension point: a registered adapter can
    back any resource kind, SQL or not, without editing the factory or core.
    """
    return ADAPTERS.register(kind)


def register_sql_dialect(name: str):
    """Register a SqlDialect compiler under a SQLAlchemy dialect name."""
    return SQL_DIALECTS.register(name)
...
def register_load_strategy(name: str):
    """Register a load strategy class under a mode name (e.g. 'upsert')."""
    return LOAD_STRATEGIES.register(name)
