...
from ..adapters.base import Adapter, WriteRequest
    ...
    def _do() -> None:
        ...
        for step in task.steps:
            ...
            if cursor and ctx.watermark_store is not None and cursor in frame.columns and not frame.empty:
                ...
                # Stage, don't commit: the cursor must not advance until the
                # downstream load has succeeded, else the next run drops rows.
                ctx.stage_watermark(task.name, cursor, high)
...
def run_load(task: LoadTask, ctx: ExecutionContext) -> None:
    def _do() -> None:
        ...
        for step in task.steps:
            ...
            keys = [step.key] if isinstance(step.key, str) else list(step.key or [])
            contract = ctx.config.schemas.get(step.input_contract) if step.input_contract else None
            requests.append(
                WriteRequest(
                    frame=frame,
                    ...
                    partition_by=list(step.write.partition_by or []),
                    scd=step.scd.model_dump() if step.scd is not None else None,
                    contract=contract,
                    as_of=as_of,
                )
            )
        # One atomic batch: all steps commit together or roll back together,
        # which also makes retrying the whole task safe (no partial double-writes).
        results = adapter.write_batch(requests)
        for result in results:
            logger.info("load '%s': %s", task.name, result.summary())

    _with_retry(task.retry, f"load '{task.name}'", _do)


def _run_as_of(ctx: ExecutionContext) -> "pd.Timestamp":
    """Logical timestamp for SCD effective dates: the partition value if the run
    is partitioned, else today (normalised)."""
    if ctx.partition_value is not None:
        try:
            return pd.Timestamp(ctx.partition_value).normalize()
        except (ValueError, TypeError):
            pass
