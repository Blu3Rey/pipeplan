...
class DocumentAdapter(Adapter):
    ...
    def _to_frame(self, docs: list[dict[str, Any]]) -> pd.DataFrame:
        if not docs:
            return pd.DataFrame()
        return pd.json_normalize(docs, sep=self.sep)

    def _nest(self, flat: dict[str, Any]) -> dict[str, Any]:
        """Reconstruct a nested document from a flat, dotted-key row, dropping
        absent (NaN/None) keys so documents stay schemaless."""
        out: dict[str, Any] = {}
    ...
    def _to_docs(self, frame: pd.DataFrame) -> list[dict[str, Any]]:
        return [self._nest(row) for row in frame.to_dict(orient="records")]
