...
class CompareNode(_Node):
    ...
    def evaluate(self, df: pd.DataFrame) -> pd.Series:
        ...
        if op is Operator.IN:
            return series.isin(self._as_list(df))
        if op is Operator.NOT_IN:
            return ~series.isin(self._as_list(df))
        if op is Operator.BETWEEN:
            lo, hi = self._as_pair(df)
        ...
        if op in (Operator.CONTAINS, Operator.STARTSWITH, Operator.ENDSWITH):
            ...
            pattern = self._resolve(df, self.value)
            if isinstance(pattern, pd.Series):
                raise ExpressionError(
                    f"operator '{op.value}' needs a scalar pattern; a bare "
                    f"${{pipe:col}} resolves to a column -- add a reducer"
                )
            if op is Operator.CONTAINS:
                return text.str.contains(str(pattern), na=False, regex=False)
            if op is Operator.STARTSWITH:
                return text.str.startswith(str(pattern), na=False)
            return text.str.endswith(str(pattern), na=False)

        value = self._resolve(df, self.value)
        # Aligned Series operands compare element-wise; scalar literals are
        # coerced to the column dtype first.
        if not isinstance(value, pd.Series):
            value = _coerce_scalar(series, value)
        comparators = {
    ...
    @staticmethod
    def _resolve(df: pd.DataFrame, value: Any) -> Any:
        """Resolve a ``${pipe:...}`` token against the flowing frame, else pass through."""
        return resolve_pipe_token(value, df) if is_pipe_ref(value) else value

    def _as_list(self, df: pd.DataFrame) -> list[Any]:
        # A pipe token may yield a list (reducer) or a column Series (use its values).
        if is_pipe_ref(self.value):
            resolved = resolve_pipe_token(self.value, df)
            if isinstance(resolved, pd.Series):
                return list(pd.unique(resolved.dropna()))
            if isinstance(resolved, (list, tuple)):
                return list(resolved)
            return [resolved]
        if not isinstance(self.value, (list, tuple)):
            raise ExpressionError(f"operator '{self.op.value}' requires a list value")
        # Elements may themselves be pipe tokens.
        return [self._resolve(df, item) for item in self.value]

    def _as_pair(self, df: pd.DataFrame) -> tuple[Any, Any]:
        values = self._as_list(df)
        if len(values) != 2:
