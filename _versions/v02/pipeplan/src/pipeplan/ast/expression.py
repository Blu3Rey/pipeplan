...
class PipeExpr(_Expr):
    """A reference to a column of the flowing frame: ``{pipe: "col"}`` (Series)
    or ``{pipe: "col|reducer"}`` (a reduced scalar/list)."""

    pipe: str

    def evaluate(self, df: pd.DataFrame) -> Any:
        from ..core.pipe import resolve_pipe_ref

        return resolve_pipe_ref(self.pipe, df)
...
def _discriminate(value: Any) -> str:
    if isinstance(value, _Expr):
        return {ColExpr: "col", LitExpr: "lit", PipeExpr: "pipe", FnExpr: "fn", BinOpExpr: "binop"}[type(value)]
