...
def _orders_archive() -> pd.DataFrame:
    """Historical orders in the same raw shape, to be stacked vertically."""
    return pd.DataFrame(
        {
            "order_id": ["ORD-0901", "ORD-0902"],
            "cust_id": [2, 3],
            "product_id": [11, 12],
            "status_code": ["C", "C"],
            "amount": ["$300.00", "$725.00"],
            "qty": [1, 4],
            "order_date": ["2025-12-20", "2025-12-28"],
            "notes": ["", "backfill"],
        }
    )
...
def main() -> None:
    ...
    with pd.ExcelWriter(xlsx_path, engine="openpyxl") as writer:
        ...
        _orders_archive().to_excel(writer, sheet_name="orders_archive", index=False)
