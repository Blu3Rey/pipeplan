        ...
        if req.partition_by:
            ...
            try:
                deleted = target.execute(
                    target.delete_matching_sql(table, req.partition_by).format(staging=target.q(staging))
                )
            finally:
                target.drop(staging)
            target.append(frame, table, req.chunksize)
...
class DeleteStrategy(LoadStrategy):
    ...
    def apply(self, target: SqlLoadTarget, req: WriteRequest) -> LoadResult:
        ...
        try:
            deleted = target.execute(
                target.delete_matching_sql(table, req.key).format(staging=target.q(staging))
            )
        finally:
            target.drop(staging)
...
class Scd2Strategy(LoadStrategy):
    ...
    def apply(self, target: SqlLoadTarget, req: WriteRequest) -> LoadResult:
        ...
        current = pd.read_sql(
            target.select_current_sql(table, keys + track, col_flag), target.conn
        ...
        try:
            updated = target.execute(
                target.close_current_sql(table, col_to, col_flag, keys).format(
                    staging=target.q(staging_keys)),
