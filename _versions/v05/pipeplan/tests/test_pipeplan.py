...
def test_native_upsert_sql_per_dialect(tmp_path):
    from pipeplan.adapters.sql import SqlLoadTarget
    a = _db(tmp_path)
    with a.engine.begin() as conn:
        t = SqlLoadTarget(conn, resource="x")
        t.dialect = "postgresql"
        pg = t.native_upsert_sql("dim", ["id", "name"], ["id"]).format(staging='"stg"')
        assert "ON CONFLICT" in pg and "excluded" in pg and "WHERE true" in pg
        t.dialect = "mysql"
        my = t.native_upsert_sql("dim", ["id", "name"], ["id"]).format(staging="`stg`")
        assert "ON DUPLICATE KEY UPDATE" in my
        t.dialect = "mssql"
        ms = t.native_upsert_sql("dim", ["id", "name"], ["id"]).format(staging="[stg]")
