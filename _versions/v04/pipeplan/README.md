...
- **Load metrics.** Each write returns rows inserted/updated/deleted, logged per
  step.
- **Dialect-native fast-paths.** `upsert` uses the native construct when the key
  is backed by a unique/primary-key constraint — `INSERT ... ON CONFLICT`
  (SQLite/Postgres), `ON DUPLICATE KEY` (MySQL), `MERGE` (SQL Server) — which is
  faster and preserves target-only columns on updated rows; it falls back to a
  portable staging `DELETE`+`INSERT` otherwise. On Postgres, bulk appends use
  `COPY` when psycopg2 is available, falling back to batched inserts.
