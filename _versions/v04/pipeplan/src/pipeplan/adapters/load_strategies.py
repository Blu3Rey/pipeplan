...
class UpsertStrategy(LoadStrategy):
    ...
    Uses the dialect-native construct (``INSERT ... ON CONFLICT`` on
    SQLite/Postgres, ``ON DUPLICATE KEY`` on MySQL, ``MERGE`` on SQL Server) when
    the key is backed by a unique/primary-key constraint -- this is faster and,
    unlike delete-then-insert, preserves target-only columns on updated rows.
    Falls back to a portable staging ``DELETE`` + ``INSERT`` (explicit column
    list) when no such constraint exists. Incoming rows are de-duplicated on the
    key (last wins) so the operation is well-defined.
    """
    ...
    def apply(self, target: SqlLoadTarget, req: WriteRequest) -> LoadResult:
        ...
        try:
            ...
            if target.supports_native_upsert(table, keys):
                sql = target.native_upsert_sql(table, cols, keys).format(staging=target.q(staging))
                target.execute(sql)
            else:
